...
sys.path.insert(0, os.path.join(base_dir, 'data_processing'))
...
class WeatherApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Система анализа погоды")
    ...
    def setup_ui(self):
        ...
        ttk.Label(input_frame, text="Город:").grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        city_entry = ttk.Entry(input_frame, textvariable=self.city_var, width=30)
        city_entry.grid(row=0, column=1, padx=5, pady=5)
        
        ttk.Label(input_frame, text="Тип анализа:").grid(row=1, column=0, sticky=tk.W, padx=5, pady=5)
        
        radio_frame = ttk.Frame(input_frame)
        radio_frame.grid(row=1, column=1, sticky=tk.W, padx=5, pady=5)
        
        ttk.Radiobutton(radio_frame, text="Погода на день", variable=self.analysis_type, 
                       value="daily", command=self.on_analysis_type_change).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(radio_frame, text="Анализ за месяц", variable=self.analysis_type, 
                       value="monthly", command=self.on_analysis_type_change).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(radio_frame, text="Анализ за год", variable=self.analysis_type, 
                       value="yearly", command=self.on_analysis_type_change).pack(side=tk.LEFT, padx=5)
      
        self.calendar_frame = ttk.Frame(input_frame)
        self.calendar_frame.grid(row=2, column=0, columnspan=2, pady=10)
        
        ttk.Label(self.calendar_frame, text="Выберите дату:").pack(side=tk.LEFT, padx=5)
        
        date_frame = ttk.Frame(self.calendar_frame)
        ...
        ttk.Label(date_frame, text="Год:").grid(row=0, column=0, padx=2)
        year_spin = ttk.Spinbox(date_frame, from_=2000, to=2100, textvariable=self.year_var, width=6)
        year_spin.grid(row=0, column=1, padx=2)
            
        ttk.Label(date_frame, text="Месяц:").grid(row=0, column=2, padx=2)
        month_spin = ttk.Spinbox(date_frame, from_=1, to=12, textvariable=self.month_var, width=4)
        month_spin.grid(row=0, column=3, padx=2)
            
        ttk.Label(date_frame, text="День:").grid(row=0, column=4, padx=2)
        day_spin = ttk.Spinbox(date_frame, from_=1, to=31, textvariable=self.day_var, width=4)
        day_spin.grid(row=0, column=5, padx=2)
        
        analyze_btn = ttk.Button(input_frame, text="Анализировать", command=self.analyze_weather)
    ...
    def on_analysis_type_change(self):
        """Обновляет интерфейс в зависимости от выбранного типа анализа"""
        pass
        
    def analyze_weather(self):
        """Основная функция анализа погоды"""
        ...
        try:
            ...
            if analysis_type == "daily":
                self.analyze_daily(city, date)
            elif analysis_type == "monthly":
                #Для месячного анализа нужна дата с первым числом месяца
                first_day = date.replace(day=1)
                self.analyze_monthly(city, first_day)
            elif analysis_type == "yearly":
                #Для годового анализа нужна дата с первым днём года
                first_day = date.replace(month=1, day=1)
                self.analyze_yearly(city, first_day)
                
        except Exception as e:
            messagebox.showerror("Ошибка", f"Произошла ошибка: {str(e)}")
            
    def analyze_daily(self, city, date):
        """Анализ погоды за день"""
        try:
            df = analyzer.analyze_daily(city, date)
            fig = self.create_daily_plot(df)
            self.display_plot(fig)
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось получить данные: {str(e)}")
            
    def analyze_monthly(self, city, date):
        """Анализ погоды за месяц"""
        try:
            daily_temp_max, daily_temp_min, daily_prec, daily_wind_speed, \
            average_values, max_values, min_values = analyzer.analyze_monthly(city, date)
            fig = self.create_monthly_plot(daily_temp_max, daily_temp_min, daily_prec, 
                                          daily_wind_speed, average_values, max_values, min_values)
            self.display_plot(fig)
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось получить данные: {str(e)}")
            
    def analyze_yearly(self, city, date):
        """Анализ погоды за год"""
        try:
            monthly_values, average_values, max_values, min_values = analyzer.analyze_yearly(city, date)
            fig = self.create_yearly_plot(monthly_values, average_values, max_values, min_values)
            self.display_plot(fig)
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось получить данные: {str(e)}")
    
    def create_daily_plot(self, df: pandas.DataFrame):
        """Создаёт график для дневных данных"""
    ...
    def create_monthly_plot(self, daily_temp_max, daily_temp_min, daily_prec, 
                           daily_wind_speed, average_values, max_values, min_values):
        """Создаёт график для месячных данных"""
        ...
        ax1.plot(days, daily_temp_max['max_temp'], 'o-', color='#ff6b6b', 
                label='Макс. температура', linewidth=2)
        ax1.plot(days, daily_temp_min['min_temp'], 'o-', color='#4ecdc4', 
                label='Мин. температура', linewidth=2)
    ...
    def create_yearly_plot(self, monthly_values, average_values, max_values, min_values):
        """Создаёт график для годовых данных"""
